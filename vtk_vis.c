...
*/			uploc[particle] = user[bi].uploc[particle];
			...
			*/
			...
			if (!interpolation) {
				...
				PetscReal beta = 1.0e-10, a_e = 0.01, wall_grid_distance= 5, normal_xp, normal_yp, normal_zp, magnitude;
				// If the particle goes outside of the pipe
				// Force the particle to sit still -> DO NOT MOVE
                        	if(radial_distance >= 0.8* radius_pipe && radial_distance <= radius_pipe)
				{
				  
				       if(i < wall_grid_distance)
				       {
					        magnitude  = sqrt(pow(csi[k][j][i].x,2)+pow(csi[k][j][i].y,2)+pow(csi[k][j][i].z,2));
                        			normal_xp  = csi[k][j][i].x/magnitude;
						normal_yp  = csi[k][j][i].y/magnitude;
						normal_zp  = csi[k][j][i].z/magnitude;
				       }

				      if(i > mx - wall_grid_distance)
				      {
					        magnitude  = sqrt(pow(csi[k][j][i].x,2)+pow(csi[k][j][i].y,2)+pow(csi[k][j][i].z,2));
                        			normal_xp  = -csi[k][j][i].x/magnitude;
						normal_yp  = -csi[k][j][i].y/magnitude;
				      ...
				      if(j <   wall_grid_distance)
				      {
					        magnitude  = sqrt(pow(eta[k][j][i].x,2)+pow(eta[k][j][i].y,2)+pow(eta[k][j][i].z,2));
                        			normal_xp  = eta[k][j][i].x/magnitude;
                        			normal_yp  = eta[k][j][i].y/magnitude;
                        			normal_zp  = eta[k][j][i].z/magnitude;
												       
				       }
				      if(j > my - wall_grid_distance)
				      {
